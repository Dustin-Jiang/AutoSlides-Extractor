...
#ifdef __GNUC__
#define TARGET_AVX512 __attribute__((target("avx512f,avx512bw,avx512vl")))
...
CPUSSIMCalculator::CPUSSIMCalculator(SIMDInstructionSet instructionSet)
  ...
  if (!detector.isSIMDSupported(instructionSet) &&
      instructionSet != SIMDInstructionSet::None) {
    m_errorMessage =
        "Requested SIMD instruction set is not supported on this platform";
    m_isReady = false;
    return;
  }
...
double CPUSSIMCalculator::calculateSSIM(const cv::Mat &img1,
                                        const cv::Mat &img2) {
  if (!m_isReady) {
    return 0.0;
  }
...
std::vector<double>
CPUSSIMCalculator::calculateBatchSSIM(const std::vector<cv::Mat> &images) {
...
double CPUSSIMCalculator::calculateSSIM_Generic(const cv::Mat &img1,
                                                const cv::Mat &img2) {
...
TARGET_SSE42
double CPUSSIMCalculator::calculateSSIM_SSE4_2(const cv::Mat &img1,
                                               const cv::Mat &img2) {
#if defined(SSE42_AVAILABLE)
...
TARGET_AVX2
double CPUSSIMCalculator::calculateSSIM_AVX2(const cv::Mat &img1,
                                             const cv::Mat &img2) {
...
TARGET_AVX512
double CPUSSIMCalculator::calculateSSIM_AVX512(const cv::Mat &img1,
                                               const cv::Mat &img2) {
...
double CPUSSIMCalculator::calculateSSIM_NEON(const cv::Mat &img1,
                                             const cv::Mat &img2) {
#ifdef __aarch64__
  double mean1, mean2, var1, var2;
...
TARGET_SSE42
void CPUSSIMCalculator::calculateMeanVariance_SSE4_2(const cv::Mat &gray,
                                                     double &mean,
                                                     double &variance) {
#if defined(SSE42_AVAILABLE)
...
TARGET_AVX2
void CPUSSIMCalculator::calculateMeanVariance_AVX2(const cv::Mat &gray,
                                                   double &mean,
                                                   double &variance) {
#if defined(AVX2_AVAILABLE)
  const double *data = reinterpret_cast<const double *>(gray.data);
  ...
  double sum_sq =
      sum_sq_array[0] + sum_sq_array[1] + sum_sq_array[2] + sum_sq_array[3];
...
TARGET_AVX512
void CPUSSIMCalculator::calculateMeanVariance_AVX512(const cv::Mat &gray,
                                                     double &mean,
                                                     double &variance) {
#if defined(AVX512_AVAILABLE)
  const double *data = reinterpret_cast<const double *>(gray.data);
...
void CPUSSIMCalculator::calculateMeanVariance_NEON(const cv::Mat &gray,
                                                   double &mean,
                                                   double &variance) {
#ifdef __aarch64__
  const double *data = reinterpret_cast<const double *>(gray.data);
...
TARGET_SSE42
double CPUSSIMCalculator::calculateCovariance_SSE4_2(const cv::Mat &gray1,
                                                     const cv::Mat &gray2,
                                                     double mean1,
                                                     double mean2) {
#if defined(SSE42_AVAILABLE)
...
TARGET_AVX2
double CPUSSIMCalculator::calculateCovariance_AVX2(const cv::Mat &gray1,
                                                   const cv::Mat &gray2,
...
TARGET_AVX512
double CPUSSIMCalculator::calculateCovariance_AVX512(const cv::Mat &gray1,
                                                     const cv::Mat &gray2,
                                                     double mean1,
                                                     double mean2) {
#if defined(AVX512_AVAILABLE)
...
double CPUSSIMCalculator::calculateCovariance_NEON(const cv::Mat &gray1,
                                                   const cv::Mat &gray2,
                                                   double mean1, double mean2) {
#ifdef __aarch64__
  const double *data1 = reinterpret_cast<const double *>(gray1.data);
...
OptimizationMetrics PerformanceBenchmark::benchmarkCalculator(
    SSIMCalculatorBase *calculator, const std::vector<cv::Mat> &testImages,
    int iterations) {

  if (!calculator || !calculator->isReady() || testImages.size() < 2) {
    OptimizationMetrics metrics;
    metrics.type =
        calculator ? calculator->getOptimizationType() : OptimizationType::None;
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    try {
      ...
      for (size_t j = 1; j < testImages.size(); ++j) {
        double ssim =
            calculator->calculateSSIM(testImages[j - 1], testImages[j]);
        // Validate result
        if (ssim < 0.0 || ssim > 1.0 || std::isnan(ssim) || std::isinf(ssim)) {
          throw std::runtime_error("Invalid SSIM result: " +
                                   std::to_string(ssim));
        }
      }
...
std::vector<cv::Mat>
PerformanceBenchmark::generateTestImages(int count, int width, int height) {
...
int PerformanceBenchmark::findBestCalculator(
    ...
    const std::vector<cv::Mat> &testImages, int iterations) {
  ...
  for (size_t i = 0; i < calculators.size(); ++i) {
    ...
    OptimizationMetrics metrics =
        benchmarkCalculator(calculators[i].get(), testImages, iterations);
...
OptimizationMetrics
PerformanceBenchmark::calculateStatistics(const std::vector<double> &timings,
  ...
  for (double timing : timings) {
    variance += (timing - metrics.averageExecutionTime) *
                (timing - metrics.averageExecutionTime);
  }
  ...
  double coefficientOfVariation =
      metrics.standardDeviation / metrics.averageExecutionTime;
...
bool OptimizationManager::initialize(const OptimizationConfig &config) {
  ...
  try {
    ...
    if (m_availableCalculators.empty()) {
      m_errorMessage =
          "No optimization calculators available for this platform";
      return false;
    }
...
void OptimizationManager::createCPUCalculators() {
  ...
  if (std::find(supportedSIMD.begin(), supportedSIMD.end(),
                SIMDInstructionSet::None) == supportedSIMD.end()) {
    try {
      auto calculator =
          std::make_unique<CPUSSIMCalculator>(SIMDInstructionSet::None);
      if (calculator->isReady()) {
        m_availableCalculators.push_back(std::move(calculator));
      }
    } catch (const std::exception &e) {
      logOptimization("Failed to create generic calculator: " +
                      std::string(e.what()));
    }
  }
}
...
void OptimizationManager::selectBestCalculator() {
  ...
  if (m_config.enableBenchmarking) {
    ...
    int bestIndex = m_benchmark->findBestCalculator(
        m_availableCalculators, testImages, m_config.benchmarkIterations);
...
double OptimizationManager::calculateSSIM(const cv::Mat &img1,
                                          const cv::Mat &img2) {
  if (!m_bestCalculator) {
    throw std::runtime_error(
        "OptimizationManager not initialized or no calculator available");
  }
...
std::vector<double>
OptimizationManager::calculateBatchSSIM(const std::vector<cv::Mat> &images) {
  if (!m_bestCalculator) {
    throw std::runtime_error(
        "OptimizationManager not initialized or no calculator available");
  }
...
std::vector<OptimizationType>
OptimizationManager::getAvailableOptimizations() const {
...
void OptimizationManager::resetToAutomatic() {
  ...
  selectBestCalculator();
  logOptimization("Reset to automatic optimization selection: " +
                  toString(m_activeOptimization));
}
...
bool OptimizationManager::runBenchmarks(int testImageCount, int iterations) {
  ...
  try {
    auto testImages =
        m_benchmark->generateTestImages(testImageCount, 1920, 1080);
    m_benchmark->findBestCalculator(m_availableCalculators, testImages,
                                    iterations);
    m_performanceMetrics = m_benchmark->getBenchmarkResults();
...
std::string OptimizationManager::toString(OptimizationType type) {
  ...
  case OptimizationType::None:
    return "None";
  case OptimizationType::SIMD_SSE4_2:
    return "SIMD SSE4.2";
  case OptimizationType::SIMD_AVX2:
    return "SIMD AVX2";
  case OptimizationType::SIMD_AVX512:
    return "SIMD AVX512";
  case OptimizationType::SIMD_NEON:
    return "SIMD NEON";
  case OptimizationType::GPU_CUDA:
    return "GPU CUDA";
  case OptimizationType::GPU_OpenCL:
    return "GPU OpenCL";
  case OptimizationType::GPU_Metal:
    return "GPU Metal";
  case OptimizationType::GPU_DirectX:
    return "GPU DirectX";
  case OptimizationType::GPU_Vulkan:
    return "GPU Vulkan";
  case OptimizationType::Hybrid_CPU_GPU:
