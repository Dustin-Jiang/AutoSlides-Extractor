...
SSIMMemoryPool::~SSIMMemoryPool() { clear(); }
...
cv::Mat SSIMMemoryPool::findOrCreateBuffer(std::vector<BufferInfo> &buffers,
                                           int width, int height, int type) {
  std::lock_guard<std::mutex> lock(m_poolMutex);

  // Try to find an available buffer with matching dimensions
  for (auto &bufferInfo : buffers) {
    if (!bufferInfo.inUse && bufferInfo.width == width &&
        bufferInfo.height == height && bufferInfo.type == type) {
...
void OptimizedSSIMCalculator::setMemoryPool(
    std::shared_ptr<SSIMMemoryPool> pool) {
  m_memoryPool = pool;
}

double OptimizedSSIMCalculator::calculateOptimizedSSIM(const cv::Mat &gray1,
                                                       const cv::Mat &gray2) {
  // Ensure images have the same dimensions
  if (gray1.size() != gray2.size()) {
    std::cerr << "Warning: Images have different dimensions in "
  ...
  if (gray1.empty() || gray2.empty()) {
    std::cerr << "Warning: Empty images in OptimizedSSIMCalculator"
              << std::endl;
    return 0.0;
  }
  ...
  try {
#if SIMD_AVAILABLE
    bool useSIMD = (gray1.type() == CV_8UC1 && gray2.type() == CV_8UC1);

#if defined(__x86_64__) || defined(_M_X64)
    if (useSIMD) {
      static const bool hasAVX2 =
          PlatformDetector::getInstance().isSIMDSupported(
    ...
    std::cerr << "Error in SIMD calculations, falling back to standard "
                 "implementation: "
              << e.what() << std::endl;
    mean1 = calculateMeanFallback(gray1);
    mean2 = calculateMeanFallback(gray2);
  ...
  if (std::isnan(mean1) || std::isnan(mean2) || std::isnan(var1) ||
      std::isnan(var2) || std::isnan(covariance)) {
    std::cerr << "Warning: NaN values detected in SSIM calculation"
              << std::endl;
    return 0.0;
  }
...
std::vector<double> OptimizedSSIMCalculator::calculateBatchSSIM(
    const std::vector<cv::Mat> &frames, bool enableDownsampling,
  ...
  try {
    ...
    for (const auto &frame : frames) {
      if (frame.empty()) {
        std::cerr << "Warning: Empty frame encountered in batch processing"
                  << std::endl;
        continue;
      }
      ...
      if (enableDownsampling &&
          (frame.cols > downsampleWidth || frame.rows > downsampleHeight)) {
        cv::resize(frame, processed,
                   cv::Size(downsampleWidth, downsampleHeight), 0, 0,
                   cv::INTER_AREA);
      }
      ...
      if (processed.channels() == 3) {
        ...
        std::cerr << "Warning: Unsupported number of channels: "
    ...
    if (processedFrames.size() < 2) {
      std::cerr << "Warning: Not enough valid frames for batch SSIM calculation"
                << std::endl;
      return results;
    }
    ...
    for (size_t i = 0; i < processedFrames.size() - 1; ++i) {
      double ssim =
          calculateOptimizedSSIM(processedFrames[i], processedFrames[i + 1]);
...
double OptimizedSSIMCalculator::calculateVarianceSIMD(const cv::Mat &grayImage,
                                                      double mean) {
  if (grayImage.type() != CV_8UC1) {
...
double OptimizedSSIMCalculator::calculateCovarianceSIMD(const cv::Mat &gray1,
                                                        const cv::Mat &gray2,
                                                        double mean1,
                                                        double mean2) {
...
double OptimizedSSIMCalculator::calculateMeanSIMD(const cv::Mat &grayImage) {
  ...
  const uint8_t *data = grayImage.ptr<uint8_t>();
  const int totalPixels = grayImage.rows * grayImage.cols;
  const int simdPixels =
      (totalPixels / 32) * 32; // Process 32 pixels at a time with AVX2
...
double OptimizedSSIMCalculator::calculateVarianceSIMD(const cv::Mat &grayImage,
  ...
  for (int i = 0; i < simdPixels; i += 8) {
    ...
    __m256 pixelsFloat =
        _mm256_cvtepi32_ps(_mm256_set_m128i(pixels32_hi, pixels32_lo));
...
double OptimizedSSIMCalculator::calculateCovarianceSIMD(const cv::Mat &gray1,
  ...
  for (int i = 0; i < simdPixels; i += 8) {
    ...
    __m128i pixels1_8 =
        _mm_loadl_epi64(reinterpret_cast<const __m128i *>(data1 + i));
    __m128i pixels2_8 =
        _mm_loadl_epi64(reinterpret_cast<const __m128i *>(data2 + i));
    ...
    __m256 pixels1Float =
        _mm256_cvtepi32_ps(_mm256_set_m128i(pixels1_32_hi, pixels1_32_lo));
    __m256 pixels2Float =
        _mm256_cvtepi32_ps(_mm256_set_m128i(pixels2_32_hi, pixels2_32_lo));
...
double
OptimizedSSIMCalculator::calculateMeanFallback(const cv::Mat &grayImage) {
  cv::Scalar meanScalar = cv::mean(grayImage);
  return meanScalar[0];
}

double
OptimizedSSIMCalculator::calculateVarianceFallback(const cv::Mat &grayImage,
                                                   double mean) {
  cv::Mat diff;
...
double OptimizedSSIMCalculator::calculateCovarianceFallback(
    const cv::Mat &gray1, const cv::Mat &gray2, double mean1, double mean2) {
  cv::Mat diff1, diff2;
...
double SSIMCalculator::calculateGlobalSSIM(const std::string &img1Path,
                                           const std::string &img2Path) {
  ...
  if (img1.empty() || img2.empty()) {
    std::cerr << "Error: Could not load images from " << img1Path << " or "
              << img2Path << std::endl;
    return 0.0;
  }
...
double SSIMCalculator::calculateGlobalSSIM(const cv::Mat &img1,
                                           const cv::Mat &img2) {
...
cv::Mat SSIMCalculator::convertToGrayscale(const cv::Mat &image) {
  ...
  if (image.channels() == 3) {
    // Convert BGR to grayscale using standard luminance formula: 0.299*R +
    // 0.587*G + 0.114*B Note: OpenCV uses BGR format, so we need to adjust the
    // formula
    cv::cvtColor(image, grayImage, cv::COLOR_BGR2GRAY);
  } else if (image.channels() == 1) {
    grayImage = image; // No need to clone for read-only operations
  } else {
    std::cerr << "Error: Unsupported number of channels: " << image.channels()
              << std::endl;
    return cv::Mat();
  }
...
cv::Mat SSIMCalculator::downsampleImage(const cv::Mat &image, int targetWidth,
                                        int targetHeight) {
  if (image.empty()) {
    return cv::Mat();
  }
  ...
  if (image.cols <= targetWidth && image.rows <= targetHeight) {
    return image; // No need to clone, caller should handle data persistence if
                  // needed
  }

  cv::Mat downsampledImage;
  cv::resize(image, downsampledImage, cv::Size(targetWidth, targetHeight), 0, 0,
             cv::INTER_AREA);
...
double SSIMCalculator::calculateMean(const cv::Mat &grayImage) {
  ...
  if (m_optimizedCalculator && grayImage.type() == CV_8UC1) {
    ...
    bool useSIMD = true;
#if defined(__x86_64__) || defined(_M_X64)
    static const bool hasAVX2 = PlatformDetector::getInstance().isSIMDSupported(
        SIMDInstructionSet::AVX2);
    useSIMD = hasAVX2;
#endif
...
double SSIMCalculator::calculateVariance(const cv::Mat &grayImage,
                                         double mean) {
  // Use optimized calculator if available and image is grayscale
  if (m_optimizedCalculator && grayImage.type() == CV_8UC1) {
#if SIMD_AVAILABLE
...
double SSIMCalculator::calculateCovariance(const cv::Mat &gray1,
                                           const cv::Mat &gray2, double mean1,
                                           double mean2) {
  // Use optimized calculator if available and images are grayscale
  if (m_optimizedCalculator && gray1.type() == CV_8UC1 &&
      gray2.type() == CV_8UC1) {
#if SIMD_AVAILABLE
    bool useSIMD = true;
#if defined(__x86_64__) || defined(_M_X64)
    ...
    if (useSIMD) {
      return m_optimizedCalculator->calculateCovarianceSIMD(gray1, gray2, mean1,
                                                            mean2);
    }
#endif
    return m_optimizedCalculator->calculateCovarianceFallback(gray1, gray2,
                                                              mean1, mean2);
  }
...
std::vector<SSIMResult>
SSIMCalculator::calculateMultiThreadedSSIM(const std::vector<SSIMTask> &tasks) {
  if (tasks.empty()) {
    return {};
  }
  ...
  const int tasksPerThread =
      (totalTasks + numThreads - 1) / numThreads; // Ceiling division

  std::vector<SSIMResult> results(totalTasks);
  std::vector<std::thread> threads;
  std::atomic<int> completedCount(0);

  // Calculate progress reporting interval (report every ~5% or at least every
  // 10 tasks)
  ...
  for (int i = 0; i < numThreads; ++i) {
    ...
    threads.emplace_back([this, &tasks, startIndex, endIndex, &results,
                          &completedCount, totalTasks, progressInterval]() {
      processBatch(tasks, startIndex, endIndex, results, completedCount,
                   totalTasks, progressInterval);
    });
  }
...
std::vector<double>
SSIMCalculator::calculateBatchSSIM(const std::vector<cv::Mat> &frames,
                                   bool enableDownsampling, int downsampleWidth,
                                   int downsampleHeight) {
  // Use optimized batch processing if available
  if (m_optimizedCalculator) {
    return m_optimizedCalculator->calculateBatchSSIM(
        frames, enableDownsampling, downsampleWidth, downsampleHeight);
  }
  ...
  for (size_t i = 0; i < frames.size() - 1; ++i) {
    double ssim =
        calculateGlobalSSIM(frames[i], frames[i + 1], enableDownsampling,
...
void SSIMCalculator::processBatch(const std::vector<SSIMTask> &tasks,
                                  ...
                                  int totalTasks, int progressInterval) {
  for (int i = startIndex; i < endIndex; ++i) {
    ...
    double score;
    if (task.useMatInput) {
      score = calculateGlobalSSIM(task.img1, task.img2, task.enableDownsampling,
                                  task.downsampleWidth, task.downsampleHeight);
    } else {
      // For file-based input, load images using Unicode-safe helper
      cv::Mat img1 =
          ImageIOHelper::imreadUnicode(task.img1Path, cv::IMREAD_COLOR);
      cv::Mat img2 =
          ImageIOHelper::imreadUnicode(task.img2Path, cv::IMREAD_COLOR);
      ...
      if (img1.empty() || img2.empty()) {
        ...
        score =
            calculateGlobalSSIM(img1, img2, task.enableDownsampling,
                                task.downsampleWidth, task.downsampleHeight);
      }
    }
    ...
    if (currentCompleted % progressInterval == 0 ||
        currentCompleted == totalTasks) {
      emit calculationProgress(currentCompleted, totalTasks);
    }
  }
}

std::vector<double> SSIMCalculator::calculateBatchSSIMFromFrameBuffers(
    const std::vector<FrameBuffer> &frameBuffers, bool enableDownsampling,
    int downsampleWidth, int downsampleHeight) {
  ...
  for (size_t i = 0; i < frameBuffers.size() - 1; ++i) {
    emit calculationProgress(static_cast<int>(i),
                             static_cast<int>(frameBuffers.size() - 1));
    ...
    if (enableDownsampling) {
      ...
      auto pooledMat1 =
          PooledMat(memoryPool->acquireWorkBuffer(
                        downsampleWidth, downsampleHeight, frame1.type()),
                    memoryPool.get(), false);
      auto pooledMat2 =
          PooledMat(memoryPool->acquireWorkBuffer(
                        downsampleWidth, downsampleHeight, frame2.type()),
                    memoryPool.get(), false);

      // Downsample frames
      cv::resize(frame1, pooledMat1.get(),
                 cv::Size(downsampleWidth, downsampleHeight), 0, 0,
                 cv::INTER_AREA);
      cv::resize(frame2, pooledMat2.get(),
                 cv::Size(downsampleWidth, downsampleHeight), 0, 0,
                 cv::INTER_AREA);
